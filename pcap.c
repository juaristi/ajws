...
get_ip_addr(pcap_if_t *dev)
{
	...
	for (cur_addr = dev->addresses; cur_addr && (!addr); cur_addr = cur_addr->next)
	{
		if (cur_addr->addr->sa_family == AF_INET)
		{
			...
			if (addr)
				memcpy(addr, cur_addr->addr, sizeof(struct sockaddr));
		}
	}
...
dev_open(pajws_dev_t dev)
{
	...
	if (pcap_activate(pcap))
	{
		...
		logprintf(LOG_FATAL, "Could not enable packet capture engine.\n");
		goto end;
	}
